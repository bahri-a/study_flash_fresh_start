...
class AppUserRepository {
  ...
  Future<void> signUpUser({required String email, required String password, required String username}) async {
    try {
      final result = await _authRepository.createAccount(email: email, password: password, username: username);
      final uid = result.user?.uid;
      if (uid == null) throw Exception("UserID konnte nicht erstellt werden");
      _authRepository.updateDisplayName(username: username);
      await _coreService.setDocument(
        path: "users",
        docId: uid,
        data: {'email': email, 'username': username, 'createdAt': FieldValue.serverTimestamp()},
      );
  ...
  Stream<AppUser?> streamCurrentAppUserData() {
    return _authRepository.authStateChanges.switchMap((firebaseUser) {
      ...
      return _coreService.streamDocument(path: 'users', docId: firebaseUser.uid).map((e) {
        final data = e.data();
  ...
  Future<AppUser?> getCurrentAppUserData() async {
    ...
    final rohDaten = await _coreService.getDocument(path: 'users', docId: firebaseUser.uid);
